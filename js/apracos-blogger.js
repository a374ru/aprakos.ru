...
var OLY = (function () {
  function OLY(year) {
    ...
    this.arrayDaysRu = ["ВОСРЕСЕНЬЕ", "ПОНЕДЕЛЬНИК", "ВТОРНИК", "СРЕДА", "ЧЕТВЕРГ", "ПЯТНИЦА", "СУББОТА",];
    this.weeks = {};
    this.easterDates = {
      2015: [3, 12],
      2016: [4, 1],
  ...
  OLY.prototype.initOLY = function () {
    ...
    if (this.theMomentTime >= new Date(this.theMomentTime.getFullYear(), this.easterDates[yearNumber][0], this.easterDates[yearNumber][1])) {
    ...
    console.log("\n" + "Прошедшая Пасха: " + this.oldEaster.toString().slice(0, 15) + "\n" + "Ожидаемая Пасха: " + this.newEaster.toString().slice(0, 16));
...
///////////////////////////////////////
    ...
    var vozdvizgenie = (this.weeks["vozdvizgenie"] = [Math.ceil((this.datesOLY.vozdvizgenieKresta[0].getTime() - this.oldEasterMLS) / 864e5 / 7), "Седмица Воздвижения",]);
    var stupkaV = (this.weeks["stupkaV"] = [Math.ceil((this.datesOLY.week24[0].getTime() - this.oldEasterMLS) / 864e5 / 7) - vozdvizgenie[0], "Воздвиженская преступка",]);
    return this.weeks;
  }
    ;
  ...
  OLY.prototype.initDatesOLY = function () {
    this.datesOLY["pentecost"] = [new Date(this.oldEasterMLS + 864e5 * 49), "Пятьдесятница",];
    this.datesOLY["vozdvizgenieKresta"] = [new Date(this.oldEaster.getFullYear() + "-09-27T00:00:00"), "Воздвижение Креста Господня",];
    this.datesOLY["zakhey"] = [new Date(this.newEasterMLS - 864e5 * 77), "Неделя Закхея",];
    this.datesOLY["mif"] = [new Date(this.newEasterMLS - 864e5 * 70), "Неделя МиФ",];
    this.datesOLY["aboutTheProdigalSon"] = [new Date(this.newEasterMLS - 864e5 * 63), "Неделя о блудном сыне",];
    this.datesOLY["aboutTheLastJudgment"] = [new Date(this.newEasterMLS - 864e5 * 56), "Неделя о страшном суде",];
    this.datesOLY["week24"] = [new Date(this.oldEasterMLS + 864e5 * 168), "17 седмица по Пятьдесятнице",];
  ...
  OLY.prototype.holydays_9 = function () {
    ...
    for (var item in this.NINEHOLIDAYS) {
      ...
      if (h9 === tmt) {
        link_to_hld9 = "/" + this.NINEHOLIDAYS[item].year + "/" + this.NINEHOLIDAYS[item].monthRU + "/" + this.NINEHOLIDAYS[item].day + ".html";
        return link_to_hld9;
        break;
      }
    }
  ...
  OLY.prototype.eventKeys = function () {
    ...
    document.addEventListener("keyup", function (event) {
      ...
      if (oneClickInfo == "KeyAKeyC") {
        alert("!!!   " + "Выполнена зачистка хранилища…".toUpperCase())
        localStorage.removeItem('ystm');
        // sessionStorage.removeItem('userDate');
        oneClickInfo = "";
      }
    });
...
// TODO #1 @a374ru - Добавить вычисления дат Великого поста и Сыропуста в скрипт 
