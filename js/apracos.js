/** @format */
// FIXME - В день Пасхи неправильно работает скрипт. Продолжает увеличивать седмицы более положенной продолжительности.
"use strict";
...
class TimeBoxOrthodox {
    constructor(userYear) {
        ...
        try {
            if (userYear != undefined) {
                ...
                console.log(
                    `Установленная дата: ${this.formatsEaster.moment
                        .toString()
                        .slice(0, 15)}`
                );
            } else {
    ...
    validate(userdate) {
        ...
        if (userdate.length > 3 && valYear >= 2016 && valYear <= 2100) {
            document
                ...
                .forEach((n) => n.classList.remove("colorBlock"));
            document
                .querySelectorAll(".seeddayON")
                .forEach((n) => n.classList.replace("seeddayON", "seedday"));
            document.getElementById(
                "zachala"
            ).innerHTML = `Зачала всего лета по Пасхе в год <span class="yearBG">${valYear} </span>`;
            document.location.replace("#");
            return `${userdate}/${this.theMoment.getMonth() + 1
    ...
    calculateDatesEaster() {
        ...
        this.formatsEaster.easter = `${this.formatsEaster.momentYear}/${this.paskhalia[this.formatsEaster.momentYear][0]
    ...
    mif() {
        this.formatsEaster.mifMLS =
            this.formatsEaster.nextEasterMLS - CONST_MLS_MiF;
        this.formatsEaster.mif = new Date(this.formatsEaster.mifMLS)
            .toString()
            .slice(0, 15);
        return `День мытаря и фарисея приходится на ${this.formatsEaster.mif}`;
    }
    voznesnieGospodne() {
        this.formatsEaster.voznesenieMLS =
            this.formatsEaster.lastEasterMLS + CONST_MLS_DAY * 39;
    ...
    pyatDesyatnica() {
        this.formatsEaster.pyatiDesyatnicaMLS =
            this.formatsEaster.lastEasterMLS + CONST_MLS_DAY * 49;
    ...
    vozdviggenieKresta() {
        ...
        this.formatsEaster.vozdviggenieMLS =
            this.formatsEaster.vozdviggenie.getTime();
    ...
    vhodGospoden() {
        this.formatsEaster.vhodMLS =
            this.formatsEaster.nextEasterMLS - CONST_MLS_DAY * 7;
        this.formatsEaster.vhod = new Date(this.formatsEaster.vhodMLS)
            .toString()
            .slice(0, 15);
    ...
    calculateLinksAll() {
        ...
        this.formatsLinks.linkToAprakosPage =
            this.formatsEaster.currentWeek +
            "/" +
            this.formatsEaster.dayNum +
            ".html";
        if (
            this.formatsEaster.currentWeek > 40 &&
            this.formatsEaster.promWeeks > 0 && this.formatsEaster.currentWeek <= this.formatsEaster.allWeeks
        ) {
    ...
    insertElements() {
        ...
        document.getElementById(
            "date5"
        ).innerHTML = `Глас седмицы - ${this.formatsEaster.glas}`;
        ...
        if (
            ...
            seedday =
                "seedday-" +
                this.formatsEaster.currentWeekStupka +
                "-" +
                this.formatsEaster.dayNum;
        }
        if (
        ...
        if (
            ...
            vvv =
                vvv + (this.formatsEaster.currentWeek - this.formatsEaster.vozStupka);
            slb = " colorBlock";
            seedday =
                "seedday-" +
