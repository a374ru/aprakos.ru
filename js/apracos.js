...
const CONST_LOG_WARNING =
  "Будте вниматильней, проверте правильность вводимой даты.";
...
class TimeBoxOrthodox {
  ...
  constructor(userYear) {
    ...
    try {
      if (userYear != undefined) {
        ...
        console.log(
          `Установленная дата: ${
            this.formatsEaster.moment.toString().slice(0, 15)
          }`,
        );
      } else {
        this.formatsEaster.moment = this.theMoment;
  ...
  validate(userdate) {
    ...
    if (userdate.length > 3 && valYear >= 2016 && valYear <= 2100) {
      document.querySelectorAll(".colorBlock").forEach((n) =>
        n.classList.remove("colorBlock")
      );
      document.querySelectorAll(".seeddayON").forEach((n) =>
        n.classList.replace("seeddayON", "seedday")
      );
      document.getElementById("zachala").innerHTML =
        `Зачала всего лета по Пасхе в год <span class="yearBG">${valYear} </span>`;
      document.location.replace("#");
      return `${userdate}/${
        this.theMoment.getMonth() + 1
  ...
  calculateDatesEaster() {
    ...
    this.formatsEaster.easter = `${this.formatsEaster.momentYear}/${
      this.paskhalia[this.formatsEaster.momentYear][0]
    }/${this.paskhalia[this.formatsEaster.momentYear][1]}`;
    ...
    this.keySystemYear =
      (this.formatsEaster.momentMLS - this.formatsEaster.easterMLS) /
      CONST_MLS_DAY;
    if (this.keySystemYear > 0) {
      var next = this.formatsEaster.momentYear + 1 + "/" +
        this.paskhalia[this.formatsEaster.momentYear + 1][0] + "/" +
        this.paskhalia[this.formatsEaster.momentYear + 1][1];
      ...
      var last = this.formatsEaster.momentYear - 1 + "/" +
        this.paskhalia[this.formatsEaster.momentYear - 1][0] + "/" +
        this.paskhalia[this.formatsEaster.momentYear - 1][1];
  ...
  mif() {
    this.formatsEaster.mifMLS = this.formatsEaster.nextEasterMLS -
      CONST_MLS_MiF;
    this.formatsEaster.mif = new Date(this.formatsEaster.mifMLS).toString()
      .slice(0, 15);
    return `День мытаря и фарисея приходится на ${this.formatsEaster.mif}`;
  }
  voznesnieGospodne() {
    this.formatsEaster.voznesenieMLS = this.formatsEaster.lastEasterMLS +
      (CONST_MLS_DAY * 39);
    this.formatsEaster.voznesenie = new Date(this.formatsEaster.voznesenieMLS)
      .toString().slice(0, 15);
    return `Вознесение приходится на ${this.formatsEaster.voznesenie}`;
  }
  pyatDesyatnica() {
    this.formatsEaster.pyatiDesyatnicaMLS = this.formatsEaster.lastEasterMLS +
      (CONST_MLS_DAY * 49);
    this.formatsEaster.pyatiDesyatnica = new Date(
      this.formatsEaster.pyatiDesyatnicaMLS,
    ).toString().slice(0, 15);
  ...
  calculateAllWeks() {
    ...
    this.formatsEaster.allWeeks = parseInt(
      (this.formatsEaster.nextEasterMLS - this.formatsEaster.lastEasterMLS) /
          CONST_MLS_DAY / 7 + "",
      10,
    );
    this.formatsEaster.promWeeks = this.formatsEaster.allWeeks - 50;
    let sss = this.formatsEaster.currentWeek =
      parseInt(
        (this.formatsEaster.momentMLS - this.formatsEaster.lastEasterMLS) /
            CONST_MLS_DAY / 7 + "",
        10,
      ) + 1;
    this.formatsEaster.beginningLentMLS = parseInt(
      (this.formatsEaster.nextEasterMLS - CONST_MLS_DAY * 48) + "",
      10,
    );
    this.formatsEaster.beginningLent = new Date(
      this.formatsEaster.beginningLentMLS,
    ).toString().slice(0, 15);
    this.formatsEaster.currentWeekStupka = sss - this.formatsEaster.promWeeks;
    let zero = parseInt(
      (this.formatsEaster.nextEasterMLS - this.formatsEaster.momentMLS) /
          CONST_MLS_DAY + "",
      10,
    );
    let today = "Сегодня";
  ...
  vozdviggenieKresta() {
    ...
    if (
      this.theMoment.getTime() <
        this.formatsEaster.mondayAfterVozdviggenie.getTime()
    ) {
      this.formatsEaster.mondayAfterVozdviggenie = undefined;
    }
    let kolichestvoSedmicPoPyatidesyatnice =
      (this.formatsEaster.vozdviggenieMLS - this.formatsEaster.lastEasterMLS) /
        CONST_MLS_DAY / 7 - 6;
    console.log(
      `Седмица на Воздвижение - ${
        Math.floor(kolichestvoSedmicPoPyatidesyatnice)
      }`,
    );
    ...
    if (
      ...
      voz =
        `Воздвижение приходится на ${kolichestvoSedmicPoPyatidesyatnice} седмицу.
      ...
      this.formatsEaster.vozStupka = stupka;
      voz =
        `Воздвижение приходится на седмицу - ${kolichestvoSedmicPoPyatidesyatnice}. Отступок нет.`;
    }
    return voz;
  }
  vhodGospoden() {
    this.formatsEaster.vhodMLS = this.formatsEaster.nextEasterMLS -
      CONST_MLS_DAY * 7;
    this.formatsEaster.vhod = new Date(this.formatsEaster.vhodMLS).toString()
      .slice(0, 15);
  ...
  calculateLinksAll() {
    ...
    if (
  this.formatsEaster.currentWeek >= 40 && this.formatsEaster.promWeeks > 0
    ) {
      ccc = this.formatsEaster.currentWeekStupka;
      this.formatsLinks.linkToAprakosPage = ccc + "/" +
        this.formatsEaster.dayNum + ".html";
      this.formatsLinks.linkToElementID2 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${
        this.formatsEaster.promWeeks + ccc
      }</a>`;
      this.formatsLinks.linkToElementID4 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${
        this.formatsEaster.promWeeks + ccc - 7
      }</a>`;
    } else if (
      ...
      this.formatsLinks.linkToElementID2 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${this.formatsEaster.currentWeek}</a>`;
      this.formatsLinks.linkToElementID4 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${
        this.formatsEaster.currentWeek - 7
      ...
      this.formatsLinks.linkToElementID2 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${ccc}</a>`;
      this.formatsLinks.linkToElementID4 = `<a href="#seedday-${
        ccc - 1
      }-7"  title="Сегодня : ${this.formatsEaster.dayName}">${ccc - 7}</a>`;
    }
  }
  insertElements() {
    this.glasSedmici();
    document.getElementById("date5").innerHTML =
      `Глас седмицы - ${this.formatsEaster.glas}`;
    ...
    if (
      ...
      this.formatsEaster.currentWeek <= 27 && this.formatsEaster.vozStupka > 0
    ) {
      vvv = vvv +
  ...
  linkToID() {
    ...
    if (anchor != "" && segmentsURL.lastIndexOf("stvol.html") == -1) {
      try {
        document.getElementById(anchor).setAttribute(
          "style",
          "cursor: pointer; color: #a55858; background-color: #f4b5ff22; padding: 0px 0.4em 0px; border-radius: 7px;",
        );
        if (
          (previosURL === null || previosURL === void 0
            ? void 0
            : previosURL.search("about.html")) != -1
        ) {
          document.getElementById(anchor).setAttribute(
            "onclick",
            'document.location.replace("../about.html"); return false',
          );
        }
      } catch (error) {
        console.log(error);
